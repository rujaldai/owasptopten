...
import com.owasptopten.insecure.comment.domain.UserComment;
...
public class UserCommentService {
    ...
    public void saveComment(@NotBlank String comment, String imageUrl, boolean isInternalComment, User user) {
        ...
        jdbcTemplate.update(
            "INSERT INTO user_comments (user_id, comment, image_url, image_data, internal, created_at) VALUES (?, ?, ?, ?, ?, NOW())",
            user.getId(),
            comment,
            imageUrl,
            imageData,
    ...
    public List<UserCommentDto> getComments(String username, User user, Pageable pageable) {
        ...
        if (ObjectUtils.isEmpty(username)) {
            return jdbcTemplate.query(
                ...
                "FROM user u JOIN user_comments uc ON u.id = uc.user_id and uc.internal = ? " +
